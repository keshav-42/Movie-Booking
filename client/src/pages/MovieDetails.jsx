  ...
  return show ? (
    <div className='px-6 md:px-16 lg:px-40 pt-30 md:pt-50'>
      <div className='flex flex-col md:flex-row gap-8 max-w-6xl mx-auto' >
        <img src={image_base_url + show.movie.poster_path} alt='poster' className='max-md:mx-auto rounded-xl h-104 max-w-70 object-cover'></img>
        <div className='relative flex flex-col gap-3'>
          ...
          <div className='flex items-center gap-2 text-gray-300'>
            <StarIcon className='w-5 h-5 text-starColor fill-starColor' />
            {show.movie.vote_average.toFixed(1)} User Rating
          </div>
          <p className='text-gray-400 mt-2 text-sm leadig-tight max-w-xl'>{show.movie.overview}</p>
      ...
      <p className='text-lg font-medium mt-20'>Your Favourtie Cast</p>
      <div className='overflow-x-auto no-scrollbar mt-8 pb-4'>
        <div className='flex items-center gap-4 w-max px-4'>
          {show.movie.casts.slice(0, 12).map((cast, index) => (
            <div key={index} className='flex flex-col items-center text-center'>
              <img src={image_base_url + cast.profile_path} alt="casts" className='rounded-full h-20 md:h-20 aspect-square object-cover' />
